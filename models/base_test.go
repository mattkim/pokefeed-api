...
import (
	"fmt"
	"os"
	"github.com/pokefeed/pokefeed-api/libstring"
...
func newDbForTest(t *testing.T) *sqlx.DB {
	...
	db, err := sqlx.Connect("postgres", fmt.Sprintf("postgres://%v@%v:%v/pokefeed-api-test?sslmode=%v", pguser, pghost, pgport, pgsslmode))
	if err != nil {
